...
async function generateWithGoogle(apiKey: string, prompt: string, settings: ImageSettings): Promise<string> {
  ...
  try {
    ...
    // Gemini 3 Pro Image and 3.1 Flash Image support imageSize (1K, 2K, 4K).
    // Flash Lite Image and Gemini 2.5 Flash Image generate at a fixed resolution.
    const supportsImageSize =
      settings.model.startsWith("gemini-3") && !settings.model.includes("flash-lite");
    ...
    const response = await ai.models.generateContent({
      model: settings.model || "gemini-3-pro-image",
      contents: prompt,
      config: {
