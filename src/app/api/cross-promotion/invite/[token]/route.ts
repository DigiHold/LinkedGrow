...
// GET /api/cross-promotion/invite/[token] - Get invite details
export async function GET(
  ...
  try {
    ...
    const [invite] = await db
      ...
      .where(eq(crossPromotionMembers.inviteToken, token))
      .limit(1);
    ...
    const [group] = await db
      .select({ name: crossPromotionGroups.name, ownerId: crossPromotionGroups.ownerId })
      .from(crossPromotionGroups)
      .where(eq(crossPromotionGroups.id, invite.groupId))
      .limit(1);
...
// POST /api/cross-promotion/invite/[token] - Accept or decline invite
export async function POST(
  ...
  try {
    ...
    if (action === "accept") {
      await db
        .update(crossPromotionMembers)
        .set({
          status: "accepted",
          userId: session.user.id,
          acceptedAt: new Date(),
        })
        .where(eq(crossPromotionMembers.id, invite.id));
