...
function InviteContent() {
  ...
  const validateInvite = async () => {
    try {
      const response = await fetch(`/api/cross-promotion/invite/${token}`);
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setSuccess(true);
        setTimeout(() => router.push("/dashboard/cross-promotion"), 2000);
      }
    } catch {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-4">
        <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-2xl shadow-lg p-8 text-center">
          ...
          <h1 className="text-xl font-bold mb-2">Cross Promotion Invite</h1>
          ...
          <div className="space-y-3">
            <Link href={`/sign-in?callbackUrl=/cross-promotion/invite?token=${token}`} className="block">
              <Button className="w-full" variant="outline">
                <LogIn className="w-4 h-4 mr-2" />
                Sign In
              </Button>
            </Link>
            <Link href={`/sign-up?callbackUrl=/cross-promotion/invite?token=${token}`} className="block">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-4">
      <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-2xl shadow-lg p-8 text-center">
        ...
        <h1 className="text-xl font-bold mb-2">Join Cross Promotion Group</h1>
        <p className="text-muted-foreground mb-6">
...
export default function CrossPromotionInvitePage() {
  return (
    <Suspense
      fallback={
