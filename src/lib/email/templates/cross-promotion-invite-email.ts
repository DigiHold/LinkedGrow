// Cross Promotion invite email template - transactional email sent via Brevo
interface CrossPromotionInviteEmailParams {
  inviterName: string;
  groupName: string;
  inviteUrl: string;
}

export function crossPromotionInviteEmailTemplate({ inviterName, groupName, inviteUrl }: CrossPromotionInviteEmailParams): string {
  return `<!DOCTYPE html>
...
<head>
    ...
    <title>${inviterName} invited you to a Cross Promotion group on LinkedGrow</title>
</head>
<body style="margin: 0 !important; padding: 0 !important; background-color: #F4F7FA;">
    <div style="background-color: #F4F7FA; line-height: 100%; font-size: 16px;">
        ...
        <div style="display: none; font-size: 0px; color: #F4F7FA; line-height: 0px; max-height: 0px; max-width: 0px; opacity: 0; overflow: hidden; mso-hide: all;">
            &nbsp;${inviterName} wants you to join their Cross Promotion group "${groupName}" on LinkedGrow.
        </div>
                        ...
                        <tr>
                            <td align="center">
                                ...
                                <table width="100%" bgcolor="#ffffff" border="0" cellspacing="0" cellpadding="0" style="color: #45556C;">
                                    ...
                                    <tr>
                                        <td class="row" style="padding: 0 50px;">
                                            <p style="font-family: 'Inter', sans-serif; color: #45556C; font-size: 16px; line-height: 165%; margin-top: 0; margin-bottom: 10px;"><strong>${inviterName}</strong> wants you to join their Cross Promotion group "<strong>${groupName}</strong>" on LinkedGrow. When group members publish LinkedIn posts, you'll get notified so you can like, comment, and repost - helping each other grow.</p>
                                        </td>
                                    </tr>
                                    <tr>
...
export function crossPromotionInviteEmailText({ inviterName, groupName, inviteUrl }: CrossPromotionInviteEmailParams): string {
  return `You've been invited!

${inviterName} wants you to join their Cross Promotion group "${groupName}" on LinkedGrow. When group members publish LinkedIn posts, you'll get notified so you can like, comment, and repost - helping each other grow.
