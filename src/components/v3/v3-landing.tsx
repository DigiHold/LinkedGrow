...
const NARROW = "mx-auto max-w-[940px] px-6";
...
const LEAD = "max-w-[58ch] text-[clamp(16.5px,1.35vw,19px)] leading-[1.58]!";
const LEAD_MUT = LEAD + " text-v3-mut dark:text-v3-mut-d";
const MONO = "font-v3-mono text-[11px] font-medium uppercase tracking-[.15em]";
...
const EM_SKY = "not-italic text-v3-sky [-webkit-text-fill-color:#8ce4f5]";
const EM_WARM =
...
const SH = "flex items-start gap-[14px]";
const SH_BUL_BASE =
...
const SH_BUL = SH_BUL_BASE + " border border-[rgba(21,93,252,.16)] bg-v3-wash dark:bg-v3-wash-d";
const SH_BUL_WARM = SH_BUL_BASE + " border border-[rgba(21,93,252,.16)] bg-v3-wash dark:border-[#3d2b23] dark:bg-[#2a1d18]";
const SH_BUL_NIGHT =
...
const FILL_MD = "rounded-[13px] px-[22px] py-3 text-[15px]";
const FILL_LG = "rounded-[14px] px-[28px] py-4 text-[16.5px]";
const FILL_SM = "rounded-[11px] px-[17px] py-[9px] text-[14px]";
const FILL_WIDE = "mt-auto w-full";
const FILL_PRI =
...
const CHROME_DOT_LT = "block h-[9px] w-[9px] rounded-full bg-v3-line2 dark:bg-v3-line2-d";
const CHROME_DOT_DK = "block h-[9px] w-[9px] rounded-full bg-[rgba(255,255,255,.16)]";
const CHROME_URL_LT =
...
const ANN = "absolute z-[5] hidden min-[1500px]:block [&_svg]:text-v3-sky [&_svg]:opacity-70";
const NOTE =
...
const PSTAT_IX = "font-v3-mono text-[10px] tracking-[.14em] text-[#c99a84] dark:text-[#8a6250]";
const PSTAT_N =
...
const PSTAT_P = "mt-3 text-[14.5px] leading-[1.55]! text-[#6d5c52] dark:text-[#a89386]";
const PSTAT_IC = "mt-5 text-[#cd8a70] dark:text-[#a3654d]";
...
const TOGGLE =
  ...
  "[&>button.on]:text-v3-ink dark:[&>button.on]:text-v3-ink-d";
const PIP =
...
const PLANS = "mx-auto grid max-w-[960px] [grid-template-columns:repeat(2,1fr)] gap-[22px] max-[860px]:[grid-template-columns:1fr]";
const PLAN =
...
const PLAN_PLAIN = "border border-v3-line2 dark:border-v3-line-d";
const PLAN_BEST =
...
const PN = "font-v3-display text-[20px] font-semibold tracking-[-.035em]";
const PD = "mt-1.5 text-[14.5px] text-v3-mut dark:text-v3-mut-d";
const PR =
...
const YR = "mt-[9px] min-h-[20px] text-[13.5px] text-v3-mut dark:text-v3-mut-d";
const WORTH =
...
const PLAN_UL = "mx-0 mb-7 mt-[22px] grid list-none gap-3 p-0 text-[15px]";
const PLAN_LI =
...
const BADGES = "mb-[14px] flex flex-wrap items-center justify-center gap-8";
const BADGE =
...
const QUOTE_BQ = "m-0 text-[17px] leading-[1.58] text-v3-ink2 dark:text-v3-ink2-d";
const QUOTE_CAP = "mt-auto flex items-center gap-3";
const QAV =
...
const SGRID = "relative z-[1] grid [grid-template-columns:repeat(2,1fr)] gap-5 max-[880px]:[grid-template-columns:1fr]";
const SC =
  ...
  "dark:border-v3-line-d dark:bg-v3-bg2-d dark:hover:bg-v3-bg2-d";
const SC_HD = "flex items-center gap-[11px] [&>svg]:flex-none [&>svg]:text-[#2ec8ea]";
const SC_H3 = "m-0 font-v3-display! text-[19.5px] font-semibold! leading-[1.2] tracking-[-.032em]! text-white";
const SC_P = "mt-3 text-[15px] text-[#93a7c5]";
const IPCHIP =
...
const WEEK_OFF = "[&_u]:after:hidden";
const ALERT =
...
const HARD_P = "mt-[14px] text-[14px] text-[#93a7c5]";
const SFOOT =
  "relative z-[1] mt-7 max-w-[76ch] border-l-2 border-[#1b62f0] pl-[18px] text-[15px] text-[#93a7c5]";

/* ── the rejected / sent message pair ── */
const MSG = "relative rounded-[20px] border p-7";
const MSG_BAD = "border-[#f5ded5] bg-[#fdf6f3] dark:border-[#3a1f24] dark:bg-[#1b1114]";
const MSG_GOOD = "border-[#cbe9dc] bg-[#f1fbf7] dark:border-[#1d3a2f] dark:bg-[#0d1a16]";
const MSG_TG =
  "mb-[18px] inline-flex items-center gap-[7px] rounded-full px-[11px] py-[5px] font-v3-mono text-[10px] uppercase tracking-[.13em]";
const MSG_TG_BAD = "bg-[#fae4dc] text-[#b03d27] dark:bg-[#3a1f24] dark:text-[#f0b8ab]";
const MSG_TG_GOOD = "bg-[#d6f1e4] text-[#0a7853]";
const MSG_BD =
...
const MCPGRID = "mt-14 grid [grid-template-columns:repeat(2,1fr)] gap-[22px] max-[900px]:[grid-template-columns:1fr]";
const MCARD =
...
const MCARD_FLIP = "justify-start [&>div:last-child]:[order:-1] [&>div:last-child]:mb-7 [&>div:last-child]:mt-0";
const MCARD_H3 =
  "mt-6 font-v3-display! text-[clamp(24px,2.6vw,31px)] font-semibold! leading-[1.2] tracking-[-.04em]! text-white";
const MCARD_P = "mt-[11px] max-w-[44ch] text-[15.5px] text-[rgba(255,255,255,.78)]";
const MTILE =
...
const CHATBODY = "grid gap-[11px] px-[15px] py-4";
const BUB = "max-w-[88%] rounded-[14px] px-[14px] py-[11px] text-[13.5px] leading-[1.5]";
const BUB_ME =
  "ml-auto rounded-br-[5px] text-white [background:linear-gradient(96deg,var(--color-v3-cyan),var(--color-v3-blue))]";
const BUB_IT = "rounded-bl-[5px] bg-[rgba(255,255,255,0.1)] text-[#eaf2fb] [&_b]:font-semibold [&_b]:text-white";
const MRUN =
  "flex items-center gap-2 px-[15px] pb-[15px] font-v3-mono text-[11px] text-[rgba(255,255,255,.55)]";
const MRUN_DOT = "h-[6px] w-[6px] flex-none animate-v3-breathe rounded-full bg-[#22c55e]";
const MCPFOOT =
...
const RULE_ON2 = RULE.replace("after:bg-v3-bg ", "after:bg-v3-bg2 ").replace(
...
const CAP_P = "mt-[13px] max-w-[46ch] text-[16px] text-v3-mut dark:text-v3-mut-d";
...
const STEP_P = "mt-[11px] max-w-[54ch] text-v3-mut dark:text-v3-mut-d";
...
const URLBAR_PROTO =
  "select-none font-v3-mono text-[14px] text-[rgba(255,255,255,.42)] max-[600px]:hidden";
const URLBAR_INPUT =
...
export function V3Landing() {
  ...
  return (
    <div
      className={
        "m-0 max-w-full scroll-smooth overflow-x-clip bg-v3-bg font-v3-sans text-[16.5px] leading-[1.6] tabular-nums text-pretty text-v3-ink antialiased dark:bg-v3-bg-d dark:text-v3-ink-d " +
        "[--e:cubic-bezier(.2,.7,.3,1)] " +
        // Preflight already zeroes margins, makes a link inherit its colour and
        // lays img/video out as blocks. Only the svg cap is ours to restate;
        // repeating the rest here would out-specify every utility below it.
        "[&_svg]:max-w-full " +
        ...
        <div className={`${WRAP} relative z-[3] text-center`}>
          ...
          <form className={`group ${URLWRAP} ${RV} mt-9`} style={{ "--d0": ".2s" } as React.CSSProperties} onSubmit={(e) => e.preventDefault()}>
            <div className={URLBAR}>
              <span className={URLBAR_PROTO}>https://</span>
              <input type="text" placeholder="yourcompany.com" aria-label="Your website" className={URLBAR_INPUT} />
              <button className={`${FILL} ${FILL_LIGHT} flex-none rounded-[13px] px-[22px] py-[14px] text-[15px] max-[600px]:w-full`} type="submit">Launch my agent for free
          ...
          <p className={`${RV} mt-[15px] text-[13.5px] text-[rgba(255,255,255,.55)] [&_b]:font-semibold [&_b]:text-[rgba(255,255,255,.85)]`} style={{ "--d0": ".28s" } as React.CSSProperties}><b>7-day free trial</b> on the Pro plan · Cancel any time · Dedicated IP and AI included</p>
          ...
          <div className={`${CAPS} ${RV}`}>
            ...
            <div className={CAP_ALT}>
              <div className={CAP_TXT_ALT}>
                ...
                <p className={CAP_P}>A reply is detected, the agent goes permanently silent for that person, and an email reaches you within the minute with their message and yours side by side. No automation ever speaks over you.</p>
                <div className={TICK}>
      ...
      <section className={`${SEC} ${MCP}`} id="mcp">
        <div className={WRAP}>
          ...
          <div className={MCPGRID}>
            <div className={`${MCARD} ${MCARD_A} ${RV_STATE}`}>
              ...
              <h3 className={MCARD_H3}>Via ChatGPT</h3>
              <p className={MCARD_P}>Find the people worth talking to and draft the messages, without leaving the chat you already have open.</p>
              <div className={CHAT}>
                <div className={CHATHEAD}><span className={CHAT_AV}><svg viewBox="0 0 24 24"><path d="M22.2819 9.8211a5.9847 5.9847 0 0 0-.5157-4.9108 6.0462 6.0462 0 0 0-6.5098-2.9A6.0651 6.0651 0 0 0 4.9807 4.1818a5.9847 5.9847 0 0 0-3.9977 2.9 6.0462 6.0462 0 0 0 .7427 7.0966 5.98 5.98 0 0 0 .511 4.9107 6.051 6.051 0 0 0 6.5146 2.9001A5.9847 5.9847 0 0 0 13.2599 24a6.0557 6.0557 0 0 0 5.7718-4.2058 5.9894 5.9894 0 0 0 3.9977-2.9001 6.0557 6.0557 0 0 0-.7475-7.0729zm-9.022 12.6081a4.4755 4.4755 0 0 1-2.8764-1.0408l.1419-.0804 4.7783-2.7582a.7948.7948 0 0 0 .3927-.6813v-6.7369l2.02 1.1686a.071.071 0 0 1 .038.052v5.5826a4.504 4.504 0 0 1-4.4945 4.4944zm-9.6607-4.1254a4.4708 4.4708 0 0 1-.5346-3.0137l.142.0852 4.783 2.7582a.7712.7712 0 0 0 .7806 0l5.8428-3.3685v2.3324a.0804.0804 0 0 1-.0332.0615L9.74 19.9502a4.4992 4.4992 0 0 1-6.1408-1.6464zM2.3408 7.8956a4.485 4.485 0 0 1 2.3655-1.9728V11.6a.7664.7664 0 0 0 .3879.6765l5.8144 3.3543-2.0201 1.1685a.0757.0757 0 0 1-.071 0l-4.8303-2.7865A4.504 4.504 0 0 1 2.3408 7.872zm16.5963 3.8558L13.1038 8.364 15.1192 7.2a.0757.0757 0 0 1 .071 0l4.8303 2.7913a4.4944 4.4944 0 0 1-.6765 8.1042v-5.6772a.79.79 0 0 0-.407-.667zm2.0107-3.0231l-.142-.0852-4.7735-2.7818a.7759.7759 0 0 0-.7854 0L9.409 9.2297V6.8974a.0662.0662 0 0 1 .0284-.0615l4.8303-2.7866a4.4992 4.4992 0 0 1 6.6802 4.66zM8.3065 12.863l-2.02-1.1638a.0804.0804 0 0 1-.038-.0567V6.0742a4.4992 4.4992 0 0 1 7.3757-3.4537l-.142.0805L8.704 5.459a.7948.7948 0 0 0-.3927.6813zm1.0976-2.3654l2.602-1.4998 2.6069 1.4998v2.9994l-2.5974 1.4997-2.6067-1.4997Z" /></svg></span>
                  <span><b>ChatGPT</b><small>LinkedGrow connected</small></span></div>
                <div className={CHATBODY}>
                  <div className={`${BUB} ${BUB_ME}`}>Show me the 10 warmest leads my agent found this week and draft a reply for each</div>
                  <div className={`${BUB} ${BUB_IT}`}>Ten leads, sorted by match. <b>Sarah Chen</b> is the strongest: she commented on a post about outreach templates two days ago. Drafts are ready for you to read.</div>
                </div>
                <div className={MRUN}><i className={MRUN_DOT}></i>read only · nothing is sent without you</div>
              </div>
            </div>

            <div className={`${MCARD} ${MCARD_FLIP} ${MCARD_B} ${RV_STATE}`}>
        ...
        <circle cx="76" cy="34" r="2.5" fill="#00e5cc" /></svg></span>
              <h3 className={MCARD_H3}>Via OpenClaw</h3>
              <p className={MCARD_P}>Run it from the agent platform you already automate with, on your own schedule, with no browser open at all.</p>
              <div className={CHAT}>
                <div className={CHATHEAD}><span className={CHAT_AV}><svg viewBox="0 0 120 120"><defs>
          <linearGradient id="oc-grad" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#ff4d4d" />
            <stop offset="100%" stopColor="#991b1b" />
          </linearGradient>
        ...
        <circle cx="76" cy="34" r="2.5" fill="#00e5cc" /></svg></span>
                ...
                <div className={CHATBODY}>
                  <div className={`${BUB} ${BUB_ME}`}>Every Monday at 8, write my week's posts and schedule them from the 1st to the 6th</div>
                  <div className={`${BUB} ${BUB_IT}`}>Scheduled. Six posts drafted in your voice, one a day, avoiding the morning you already have something booked.</div>
                </div>
                <div className={MRUN}><i className={MRUN_DOT}></i>runs on a cron · you approve before anything publishes</div>
              </div>
            </div>

            <div className={`${MCARD} ${MCARD_FLIP} ${MCARD_C} ${RV_STATE}`}>
      ...
      <section className={`${SEC} ${NIGHT} v3-night`} id="safety">
        ...
        <div className={WRAP}>
          ...
          <div className={HARD}>
            ...
            <div className={`${RV_STATE} ${HARD_CELL}`} style={{ "--d0": ".07s" } as React.CSSProperties}><b className={HARD_B}>1</b><p className={HARD_P}>dedicated residential IP per LinkedIn account</p></div>
            <div className={`${RV_STATE} ${HARD_CELL}`} style={{ "--d0": ".14s" } as React.CSSProperties}><b className={HARD_B}>40-120<em className="not-italic text-[.44em] tracking-[-.01em]">s</em></b><p className={HARD_P}>enforced gap between any two actions</p></div>
            <div className={`${RV_STATE} ${HARD_CELL}`} style={{ "--d0": ".21s" } as React.CSSProperties}><b className={HARD_B}>0</b><p className={HARD_P}>actions on weekends, public holidays included</p></div>
          </div>
        ...
        <div className={WRAP}>
          ...
          <div className={`${FAQSIDE} ${RV}`}>
            ...
            <div className={ASKCARD}>
              ...
              <a className={`${FILL} ${FILL_LIGHT} ${FILL_SM} mt-[18px]`} href="mailto:contact@linkedgrow.ai">Email us directly
          ...
          <div className={`${FAQ} ${RV}`} id="faq" style={{ "--d0": ".08s" } as React.CSSProperties}>
            <div className={Q}><button className={Q_BTN}><span className={Q_IX}>01</span>Will this get my LinkedIn account banned?<span className={Q_PM}>+</span></button>
              <div className="a max-h-0 overflow-hidden [transition:max-height_.44s_var(--ease-v3)]"><p className={Q_A_P}>Nobody in this category can honestly promise it never happens. What we do is start slow, keep human hours, give your account an address nobody else touches, and stop everything the second LinkedIn asks a question. All of it sits on your dashboard rather than hidden in a settings page, so you can watch the guardrails doing their job.</p></div></div>
            <div className={Q}><button className={Q_BTN}><span className={Q_IX}>02</span>Do I need to bring my own AI key?<span className={Q_PM}>+</span></button>
              <div className="a max-h-0 overflow-hidden [transition:max-height_.44s_var(--ease-v3)]"><p className={Q_A_P}>Not for the lead generation, which runs on our AI and is included in the price with no credits to run out of. If you also want it writing your posts, that half uses your own key, and that is exactly what keeps generation unlimited for a few dollars a month.</p></div></div>
            <div className={Q}><button className={Q_BTN}><span className={Q_IX}>03</span>What actually happens when somebody replies?<span className={Q_PM}>+</span></button>
              <div className="a max-h-0 overflow-hidden [transition:max-height_.44s_var(--ease-v3)]"><p className={Q_A_P}>The agent goes permanently silent for that person and emails you within the minute, with their reply, who they are, and the messages you had already sent, so the conversation makes sense without scrolling anywhere. From that point it is yours and the automation never speaks over you.</p></div></div>
            <div className={Q}><button className={Q_BTN}><span className={Q_IX}>04</span>How is this different from every other outreach tool?<span className={Q_PM}>+</span></button>
              <div className="a max-h-0 overflow-hidden [transition:max-height_.44s_var(--ease-v3)]"><p className={Q_A_P}>Three things, and all of them are checkable. Every lead shows you the real post it came from, so you can verify the reasoning yourself. Every message passes a gate that rejects AI-sounding writing before it is allowed to send. And the safety system is visible rather than buried, because your account is worth more than any single campaign.</p></div></div>
            <div className={Q}><button className={Q_BTN}><span className={Q_IX}>05</span>How long before I see something?<span className={Q_PM}>+</span></button>
              <div className="a max-h-0 overflow-hidden [transition:max-height_.44s_var(--ease-v3)]"><p className={Q_A_P}>Leads appear within the first hour. Invitations start the same day at a deliberately low volume, and the first replies usually land in the second week once warm-up has raised the pace. Anyone promising results on day two is skipping the part that keeps your account alive.</p></div></div>
            <div className={Q}><button className={Q_BTN}><span className={Q_IX}>06</span>Can I still use LinkedGrow for writing posts?<span className={Q_PM}>+</span></button>
              <div className="a max-h-0 overflow-hidden [transition:max-height_.44s_var(--ease-v3)]"><p className={Q_A_P}>Yes, and it is in every plan. The generator, the editor, the calendar, carousels and the hook library are all still there, trained on your voice, and they run on your own AI key. The agent handles the outbound half while the publishing half carries on exactly as it does today.</p></div></div>
          </div>
          </div>
        </div>
      </section>
