...
test("rejects unresolved placeholder", () => {
  const msg = "Hey [firstname], you post a lot about AI search and I built a checker for exactly that today. Best, Jane Doe";
  assert.equal(validateMessage(msg, ctx).ok, false);
});

test("rejects em dash", () => {
  const msg = "Hey Alex, you post about AI search — I built a checker for exactly that and would show you. Best, Jane Doe";
...
test("rejects banned word", () => {
  const msg = "Hey Alex, I think we could leverage your audience and I built a checker for AI search you might like. Best, Jane Doe";
  const r = validateMessage(msg, ctx);
  assert.equal(r.ok, false);
...
test("rejects generic slop opener", () => {
  const msg = "I saw your comment on that post and wanted to connect because I built a checker for AI search you might like. Best, Jane Doe";
  const r = validateMessage(msg, ctx);
  assert.equal(r.ok, false);
...
test("rejects headline dump", () => {
  const msg = "Hey Alex, since you are VP of Marketing at Cloudstack scaling B2B growth, I built a checker for AI search you might like. Best, Jane Doe";
  const r = validateMessage(msg, ctx);
  assert.equal(r.ok, false);
...
test("rejects too long", () => {
  ...
  const msg = `Hey Alex, ${filler} so anyway that is the idea. Best, Jane Doe`;
  assert.equal(validateMessage(msg, ctx).ok, false);
});
